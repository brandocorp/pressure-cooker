...
require 'pressure_cooker/cooker/core/subcommand_loader'
...
class PressureCooker
  class Cooker
    ...
    def self.subcommand_loader
      @subcommand_loader ||= Cooker::SubcommandLoader.new(chef_config_dir)
    end
    ...
    # Print the list of subcommands cooker knows about. If +preferred_category+
    # is given, only subcommands in that category are shown
    def self.list_commands(preferred_category=nil)
      load_commands

      category_desc = preferred_category ? preferred_category + " " : ''
      msg "Available #{category_desc}subcommands: (for details, cooker SUB-COMMAND --help)\n\n"
    ...
    # Run cooker for the given +args+ (ARGV), adding +options+ to the list of
    # CLI options that the subcommand knows how to handle.
    # ===Arguments
    # args::: usually ARGV
    # options::: A Mixlib::CLI option parser hash. These +options+ are how
    # subcommands know about global cooker CLI options
    ...
    def self.subcommand_class_from(args)
      ...
      # see if we got the command as e.g., cooker workflow-start
    ...
    OFFICIAL_PLUGINS = %w[cookbook databag]
    ...
    def initialize(argv=[])
      ...
      # cooker node run_list add requires that we have extra logic to handle
    ...
    def run_with_pretty_exceptions
      unless self.respond_to?(:run)
        ui.error "You need to add a #run method to your cooker command before you can use it"
      end
      enforce_path_sanity
      run
    rescue Exception => e
    ...
    def humanize_exception(e)
      ...
      when Errno::ECONNREFUSED, Timeout::Error, Errno::ETIMEDOUT, SocketError
        ...
        ui.info "Check your cooker configuration and network settings"
      when NameError, NoMethodError
        ui.error "cooker encountered an unexpected error"
        ui.info  "This may be a bug in the '#{self.class.common_name}' cooker command or plugin"
